...
import { deviceSupplyWater, deviceSupplyWaterExport } from 'comm/src/services'
import {
  getTimestampType,
  getQuarterDate,
  formatTime,
...
export default {
      ...
      formList: [
        {
          ...
          custom: () => {
            return (
              <el-radio-group
                v-model={this.form.queryType}
                size="mini"
                // change={this.switchDateType()}
              >
      ...
      chartConfig: {
        ...
        legend: {
          marker: (name, index, item) => {
            return {
              symbol: name === '最小流量' ? 'hyphen' : 'square',
              style: {
                lineWidth: 1
              }
            }
  ...
  methods: {
    async siteDataBtn() {
      ...
      this.loading = true
      let { code, data } = await deviceSupplyWater(parames)
      if (code === 200) {
        this.loading = false
    ...
    echartInit(data) {
      ...
      dv.transform({
        type: 'fold',
        fields: this.form.queryType === '1' ? ['supplyingWater', 'minimumFlow'] : ['supplyingWater'],
        key: 'type',
        value: 'value'
      })
      dv.rows.map(res => {
        if (res.type === 'supplyingWater') {
          ...
          res.type = '最小流量'
          res.minimumFlow = res.value ? res.value : 0
          res.value = null
        }
      })
