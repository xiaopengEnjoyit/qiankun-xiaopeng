...
import { areaDmaTotalDifference, areaDmaTotalDifferenceExport } from 'comm/src/services'
...
export default {
      ...
      formList: [
        {
          ...
          custom: () => {
            return (
              <el-radio-group v-model={this.form.queryType} size="mini" border>
  ...
  methods: {
    async siteDataBtn() {
      ...
      this.loading = true
      let { code, data } = await areaDmaTotalDifference(parames)
      if (code === 200) {
        this.loading = false
        ...
        this.tableDataColumns.exportUrlParames = {
          url: areaDmaTotalDifferenceExport,
          parames: parames
        }
    ...
    echartInit(data) {
      ...
      dv.transform({
        type: 'fold',
        fields: ['supplyingWater', 'useWater', 'totalDifferenceRate'],
        key: 'type',
        value: 'value'
      })
      dv.rows.map(res => {
    ...
    handleQuery(form) {
      // this.form.dateArr = form.dateArr
      this.siteDataBtn()
    }
  },
  watch: {
